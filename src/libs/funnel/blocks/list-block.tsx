    ...
    <ul className="grid grid-cols-1 @md:grid-cols-2 gap-8 @md:gap-12">
      {block.items.map((item) => (
        <li key={item.id} className={`flex flex-col items-center text-center ${textColor}`}>
          ...
          <h3 title={item.title} className="text-xl @md:text-2xl font-semibold mb-2 break-words">{item.title}</h3>
          {item.description && <p title={item.description} className="leading-relaxed break-words">{item.description}</p>}
        </li>
      ))}
    </ul>
  );
